...
def mongo_connection():
    try:
        mongo_uri = "mongodb://iotlabRoot:" + urllib.parse.quote("wsnlab123@") + "@150.140.193.156"
        myclient = MongoClient(mongo_uri, 27017)
        # connect to the database "covid"
        covid_db = myclient["covid"]
        # connect to the collection "covid_greek"
        covid_greek_col = covid_db["covid_greek"]
        print('Database connection is successful!')
    except Exception as e:
        print("Something went wrong and no connection is allowed:", e)
    return covid_greek_col
